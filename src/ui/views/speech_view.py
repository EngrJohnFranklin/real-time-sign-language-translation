...
class SpeechView(ctk.CTkFrame):
    def __init__(self, parent, on_back, **kwargs):
        ...
        status_card = ctk.CTkFrame(body, fg_color=COLOR_CARD, corner_radius=CORNER)
        status_card.pack(fill="x", pady=(0, PAD_CARD))
        ctk.CTkLabel(
        ...
        self._speech_status = ctk.CTkLabel(
            ...
            corner_radius=CORNER, height=48,
        )
        self._speech_status.pack(fill="x", padx=PAD_CARD, pady=(0, PAD_CARD))

        text_card = ctk.CTkFrame(body, fg_color=COLOR_CARD, corner_radius=CORNER)
        text_card.pack(fill="x", pady=(0, PAD_CARD))
        ctk.CTkLabel(
        ...
        self._text_display = ctk.CTkLabel(
            ...
            corner_radius=CORNER, height=100, wraplength=540,
        )
        self._text_display.pack(fill="x", padx=PAD_CARD, pady=(0, PAD_CARD))

        self._speak_button = ctk.CTkButton(
            body, text="Speak", font=FONT_BUTTON,
            height=52, corner_radius=CORNER, fg_color=COLOR_PRIMARY,
            hover_color=COLOR_PRIMARY_HOVER,
        )
        self._speak_button.pack(pady=PAD_CARD, padx=PAD_CARD, fill="x")
